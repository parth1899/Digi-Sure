...
const AdminPolicies = () => {
  ...
  const [selectedDoc, setSelectedDoc] = useState<{ imageUrl: string; fileName: string } | null>(null);
            ...
            {policies.map((policy) => (
              <React.Fragment key={policy.id}>
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatCurrency(policy.policyDetails.premiumBreakdown.total)}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span className={`text-sm ${policy.forgeryScore > 50 ? "text-red-600" : "text-green-600"}`}>
                        {Math.round(policy.forgeryScore)}%
                      </span>
                      {policy.forgeryScore > 50 && <AlertTriangle size={16} className="ml-2 text-red-500" />}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex space-x-2">
                      <button onClick={() => updatePolicyStatus(policy.id, "Active")} className="text-green-600 hover:text-green-700">
                        <Check size={20} />
                      </button>
                      <button onClick={() => updatePolicyStatus(policy.id, "Rejected")} className="text-red-600 hover:text-red-700">
                        <X size={20} />
                      </button>
                    </div>
                  </td>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              {policy.documents.map((doc, index) => (
                                <div
                                  ...
                                  onClick={() => setSelectedDoc({ imageUrl: doc.imageUrl, fileName: doc.fileName })}
                                >
                                  <div className="flex flex-col items-center">
                                    <div className="w-40 h-40 bg-gray-200 overflow-hidden flex justify-center items-center">
                                      <img src={doc.imageUrl} alt={doc.fileName} className="object-cover w-full h-full" />
                                    </div>
                                    <div className="mt-2 text-center">
                                      <p className="text-gray-900 font-medium text-sm">{doc.fileName}</p>
                                      <p className="text-gray-500 text-xs">{doc.label}</p>
                                      <p className={`text-xs ${doc.forgeryScore > 50 ? "text-red-600" : "text-green-600"}`}>
      ...
      {selectedDoc && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white p-6 rounded-lg z-10 max-w-md mx-auto">
            ...
            <img src={selectedDoc.imageUrl} alt={selectedDoc.fileName} className="w-full h-auto rounded" />
          </div>
        </div>
      )}
    </div>
